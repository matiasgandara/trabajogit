...
class CasinoView {
    ...
    public function displayCasino($casino) {
        $contenido = "<table class='table table-hover table-sm''> <thead class='thead-dark'> <tr>
            <th scope='col'> Juego </th> 
            <th scope='col'> Fecha </th>
            <th scope='col'> Monto </th>
        </tr> </thead> <tbody>";
        foreach ($casino as $objCasino) {
            $contenido .= "<tr>
            <td> {$objCasino->nombre} </td> 
            <td> {$objCasino->fecha} </td>
            <td> {$objCasino->monto} </td>
            </tr>";
        }
        $contenido .= "</tbody> </table>";
        $contenido .= '<form action="juegos" method="post">
